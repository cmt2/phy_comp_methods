...
The character state starts at 0 then changes by the amount (x). Imagine this a size of some trait, like leaf length. when x is negative, it means it got smaller, and when positive, it got bigger.
    ...
    We can build up to this by starting to generate a line of trait change over time. Run these lines of code one at a time (not as a whole block). Each point represents the value of the trait at a new point in time. 
...
Notice that this block of code does the same thing we did before but plotting several independent lineages all at once. We will 
```{r}
t <- 0:100
sig2 <- 0.01
nsim <- 100
...
lines(t, X[1, ], xlab = "time", ylab = "phenotype", ylim = c(-2, 2), col = "red")
```
